...
if(count($list)>0){?>
<div id="btcontentslider<?php echo $module->id; ?>" style="display:none;width:<?php echo $moduleWidthWrapper;?>" class="defaultLayout  bt-cs<?php echo $moduleclass_sfx? ' '.$params->get('moduleclass_sfx'):'';?>">
		...
		<?php foreach( $list as $i => $row ): ?>
			<div class="bt-row <?php if($i==0) echo 'bt-row-first'; else if($i==count($list)-1) echo 'bt-row-last' ?>"  style="width:<?php echo $itemWidth;?>%" >
				...
				<?php if( $row->thumbnail && $align_image != "center"): ?>
					<div class="image-align-<?php echo $align_image;?>">
						<a target="<?php echo $openTarget; ?>" class="bt-image-link<?php echo $modal? ' modal':''?>" title="<?php echo $row->title;?>" href="<?php echo $modal?$row->mainImage:$row->link;?>">
						  <img <?php echo $imgClass ?>  src="<?php echo $row->thumbnail; ?>" alt="<?php echo $row->title?>"  style="width:<?php echo $thumbWidth ;?>px; float:<?php echo $align_image;?>;" title="<?php echo $row->title?>" />
						</a>
					</div>
				<?php endif ; ?>
				...
				<?php if($show_category_name_as_link) : ?>
					...
					<?php if( $showAuthor ): ?>
						...
						<span class="lastest_sidebar"><?php echo JHTML::_('date', $row->created , 'l, m, Y'); ?></span>
						<?php endif; ?>
					</div>
					<?php endif; ?>
