...
<form name="product" method="post" action="<?php print $this->action?>" enctype="multipart/form-data" autocomplete="off">
    ...
    <h1 class="productName"><?php print $this->product->name?>
	
	<!--<?php //if ($this->config->show_product_code){?> <span class="jshop_code_prod">(<?php //print _JSHOP_EAN?>: <span id="product_code"><?php //print $this->product->getEan();?></span>)</span><?php //}?>-->
			...
			<div class="col-md-5 col-sm-6 col-xs-12">
				<div class="productDetailImage">
					<div class="image_middle">
						...
						<a class="lightbox bigImage" id="main_image_full_<?php print $image->image_id?>" href="<?php print $this->image_product_path?>/<?php print $image->image_full;?>" <?php if ($k!=0){?>style="display:none"<?php }?> title="<?php print htmlspecialchars($image->_title)?>">
							<img class="jShopMainImg" id = "main_image_<?php print $image->image_id?>" src = "<?php print $this->image_product_path?>/<?php print $image->image_name;?>" alt="<?php print htmlspecialchars($image->_title)?>" title="<?php print htmlspecialchars($image->_title)?>" />
							<!--<div class="text_zoom">
					...
					<div class="jshop_img_description">
						...
						<div class="list_product_image_thumb_inner">
							<?php if ( (count($this->images)>1) || (count($this->videos) && count($this->images)) ) {?>
								<?php foreach($this->images as $k=>$image){?>
								<div class="jShopThumImg">
				...
				<div class="col-md-7 col-sm-6 col-xs-12 productColRight">
					<div class="productInfoTitle"><?php echo JText::_('JSHOP_PRODUCT_INFORMATION'); ?></div>
					
					<?php if ($this->product->_display_price){?>
						<div class="prod_price">
							<span class="textLeft"><?php print _JSHOP_PRICE?></span>
					...
					<?php print $this->_tmp_product_html_before_buttons;?>
						<?php if (!$this->hide_buy){?>                         
							...
							<div>
								<!--<div class="prod_qty">
									<span class="textLeft"><?php print _JSHOP_QUANTITY?></span>
								</div>-->
								<div class="prod_qty_input">
								<span class="textLeft"><?php print _JSHOP_QUANTITY?></span><span class="splitTextLeft">:</span>
									...
									<input type="submit" class="button addCart" value="<?php print _JSHOP_ADD_TO_CART?>" onclick="jQuery('#to').val('cart');" />
									<?php if ($this->enable_wishlist){?>
										<input type="submit" class="button addWishList" value="" onclick="jQuery('#to').val('wishlist');" />
									<?php }?>
						...
						<div class="jshop_prod_attributes">
							...
							<?php foreach($this->attributes as $attribut){?>
								...
								<div class="productAttributes">
									...
									<span id='block_attr_sel_<?php print $attribut->attr_id?>' class="productAttributesSelect">
	...
	<div class="productDesc"><?php echo JText::_('JSHOP_PRODUCT_DESC'); ?></div>
    ...
    <div class="prod_free_attribs">
        ...
        <div>
            <div class="name"><span class="freeattribut_name"><?php print $freeattribut->name;?></span> <?php if ($freeattribut->required){?><span>*</span><?php }?><span class="freeattribut_description"><?php print $freeattribut->description;?></span></div>
            <div class="field"><?php print $freeattribut->input_field;?></div>
        </div>
        <?php }?>
        </table>
