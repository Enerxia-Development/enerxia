...
$document->addStyleSheet(  JURI::root().'templates/'.$template.'/html/mod_bt_contentslider/css/layout_custom.css');
$document->addScript(  JURI::root().'templates/'.$template.'/html/mod_bt_contentslider/js/slides2.js');
$document->addScript(  JURI::root().'templates/'.$template.'/html/mod_bt_contentslider/js/custom.js');


if(count($list)>0){?>
<div id="btcontentsliderr<?php echo $module->id; ?>" style="display:none;width:<?php echo $moduleWidthWrapper;?>" class="bt-cs-custom<?php echo $moduleclass_sfx? ' bt-cs'.$params->get('moduleclass_sfx'):'';?>">
	
	
	
	<div class="swap-title">
		<div class="swap-title-inner">
				<?php 
					//$add_style = "";
				...
				<?php } ?>
				<?php if($params->get('desc_custom_layout')) { ?>
					<div class="desc_custom_layout"><?php echo $params->get('desc_custom_layout') ?> </div>
				<?php  }?>	
				<?php if( $next_back && $totalPages  > 1  ) : ?>
				<div class="btcontentsliderrDirection">
				...
				<?php foreach( $list as $i => $row ): ?>
					<div class="bt-row <?php if($i==0) echo 'bt-row-first'; else if($i==count($list)-1) echo 'bt-row-last' ?>"  style="width:<?php echo $itemWidth;?>%" >
						...
						<?php if( $row->thumbnail && $align_image != "center"): ?>
							<div class="bt-image-align-<?php echo $align_image;?>">
...
<?php 
	...
	$document->addStyleDeclaration(
		'
		@media screen and (max-width: 480px){.bt-cs-custom .bt-row{width:100%!important;}}'
	);
	}
}
else
